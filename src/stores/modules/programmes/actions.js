import { FETCH_LIST as FETCH_LIST_PROGRAMME, GET as GET_PROGRAMME, STORE as STORE_PROGRAMME, UPDATE as UPDATE_PROGRAMME, DESTROY as DESTROY_PROGRAMME } from "../../actions.type";

import { SET_LIST as SET_LIST_PROGRAMME, FIND as findProgramme, ADD as ADD_PROGRAMME, MODIFY as MODIFY_PROGRAMME, REMOVE as REMOVE_PROGRAMME } from "../../mutations.type";
...
export default {
  FETCH_LIST_PROGRAMME: async ({ commit }) => {
    ...
    if (status === 200 || status === 201) commit(SET_LIST_PROGRAMME, data.data);
  ...
  async GET_PROGRAMME({ commit, getters }, { key, value }) {
    ...
    if ((programme === null || programme || undefined) && typeof value === "int") {
      ...
      if (status === 200 || status === 201) programme = data.data;
    }
  ...
  async STORE_PROGRAMME({ commit }, payload) {
    ...
    if (status === 200 || status === 201) commit(ADD_PROGRAMME, data.data);
  ...
  async UPDATE_PROGRAMME({ commit }, { programme, id }) {
    ...
    if (status === 200 || status === 201) commit(MODIFY_PROGRAMME, data.data);
  ...
  async DESTROY_PROGRAMME({ commit }, idProgramme) {
    ...
    if (status === 200 || status === 201 || status === 204) commit(REMOVE_PROGRAMME, idProgramme);
