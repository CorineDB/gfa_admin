...
export default {
  ...
  methods: {
    ...
    supprimer() {
      this.deleteProgramme(this.programmeId).then((response) => {
    ...
    sendForm() {
      ...
      //this.programme?.id
      if (this.update) {
        this.updateProgramme({ programme: this.formModifierProgramme, id: this.programme?.id })
          .then((response) => {
        ...
        this.addProgrammeLoader = true;
        this.saveProgramme(this.formAjoutProgramme)
          .then((response) => {
            if (response.status == 200 || response.status == 201) {
              this.addProgrammeLoader = false;
...
<template>
  ...
  <div class="grid grid-cols-12 gap-6 mt-5">
    <div class="col-span-12 flex justify-between items-center">
      <div class="w-full sm:w-auto my-3 sm:mt-0 sm:ml-auto md:ml-0">
        <div class="w-56 relative text-slate-500">
          <input type="text" class="form-control w-56 box pr-10" v-model="search" placeholder="Recherche..." />
    ...
    <div v-for="(program, index) in paginatedAndFilteredData" :key="index" class="intro-y col-span-12 md:col-span-6 lg:col-span-4 _xl:col-span-3">
      <div class="box" v-if="program">
        ...
        <div class="flex justify-center lg:justify-end items-center p-5 border-t border-slate-200/60 dark:border-darkmode-400">
          ...
          <a class="flex items-center text-danger" href="javascript:;" @click="deletePrograms(program)"> <Trash2Icon class="w-4 h-4 mr-1" /> Supprimer </a>
        </div>
      </div>
    </div>
  ...
  <Modal size="modal-xl" :show="showModal" @hidden="showModal = false">
    ...
    <ModalBody v-if="!update" class="grid grid-cols-12 gap-4 gap-y-3">
      ...
      <div class="col-span-12 sm:col-span-6">
        ...
        <p class="text-red-500 text-[12px] mt-2 col-span-12" v-if="messageErreur.code">{{ messageErreur.code }}</p>
      </div>
      <div class="col-span-12 sm:col-span-6">
        <InputForm v-model="formAjoutProgramme.debut" class="col-span-12" type="date" :required="true" placeHolder="Entrer la date de début" label="Début du programme" />
      ...
      <div class="col-span-12 sm:col-span-6">
        ...
        <InputForm v-model="formAjoutProgramme.fin" class="col-span-12" type="date" :required="true" placeHolder="Entrer la date de fin du programme" label="Fin du programme" />
      ...
      <div class="col-span-12 sm:col-span-6">
        <label for="modal-form-5" class="form-label">Objectifs globaux</label>
        <textarea id="validation-form-6" class="form-control" v-model="formAjoutProgramme.objectifGlobaux" placeholder="Ecriver les objectifs globaux du projet"></textarea>
        ...
        <p class="text-red-500 text-[12px] mt-2 col-span-12" v-if="messageErreur.objectifGlobaux">{{ messageErreur.objectifGlobaux }}</p>
      </div>
    </ModalBody>
    <ModalBody v-else class="grid grid-cols-12 gap-4 gap-y-3">
      ...
      <div class="col-span-12 sm:col-span-6">
        <InputForm v-model="formModifierProgramme.debut" class="col-span-12" type="date" :required="true" placeHolder="Entrer la date de début" label="Début du programme" />
      ...
      <div class="col-span-12 sm:col-span-6">
        <InputForm v-model="formModifierProgramme.fin" class="col-span-12" type="date" :required="true" placeHolder="Entrer la date de fin du programme" label="Fin du programme" />
